...
def run_single_experiment(
    ...
    """Train one DDPG algorithm-configuration pair from the 24-cell benchmark grid."""
    set_global_seed(GLOBAL_SEED)

    u_flag = int(include_u_abs)
    b_flag = int(use_bonus_penalty)

    base_name = f"ddpg_continuum_robot_s{state_type}u{u_flag}r{reward_type}b{b_flag}"
...
if __name__ == "__main__":
    ...
    if curr != "DDPG_learning":
        print("Tip: run this script from the DDPG_learning directory for consistent paths.")
    main()
