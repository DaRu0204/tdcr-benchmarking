...
def run_single_experiment(
    ...
    """Train one TD3 algorithm-configuration pair from the 24-cell benchmark grid."""
    set_global_seed(GLOBAL_SEED)

    u_flag = int(include_u_abs)
    b_flag = int(use_bonus_penalty)

    base_name = f"td3_continuum_robot_s{state_type}u{u_flag}r{reward_type}b{b_flag}"
    ...
    noise = OUNoise(env.action_dim, theta=0.15, sigma=0.2, max_action=env.max_action)
    ...
    for episode in range(total_episodes):
        ...
        while not done:
            if total_steps < start_timesteps:
                action_noisy = np.random.uniform(
                    -env.max_action, env.max_action, size=env.action_dim
                )
            else:
                action = agent.select_action(state)

                action_noisy = np.clip(
                    action + noise.sample(), -env.max_action, env.max_action
                )

            next_state, reward, done, truncated = env.step(action_noisy)

            buffer.add(
                state, action_noisy, next_state, reward, float(done), float(truncated)
            )
...
if __name__ == "__main__":
    ...
    if curr != "TD3_learning":
        print("Tip: run this script from the TD3_learning directory for consistent paths.")
    main()
